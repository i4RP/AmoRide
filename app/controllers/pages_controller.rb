  ...
  def search
    ...
    if params[:q].present?
      ...
      if params[:q][:home_type_eq_any].present?
        ...
        session[:House] = session[:home_type_eq_any].include?("ママチャリ")
        session[:Mansion] = session[:home_type_eq_any].include?("クロスバイク")
        session[:Apartment] = session[:home_type_eq_any].include?("ロードバイク")
      else
